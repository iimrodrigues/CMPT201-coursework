...
int main() {
  ...
  if (pubkey == NULL) {
    handle_error("PEM_read_PUBKEY failed");
  }
...
#define MAX_FILE_SIZE 512
  ...
  if (EVP_DigestVerifyInit(ctx, NULL, EVP_sha256(), NULL, pubkey) == 0) {
    handle_error("VerifyInit failed");
  }
  if (EVP_DigestVerifyUpdate(ctx, message, msg_size) == 0) {
    handle_error("VerifyUpdate failed");
  }

  int verification = EVP_DigestVerifyFinal(ctx, signature, sign_size);
  EVP_MD_CTX_free(ctx);
